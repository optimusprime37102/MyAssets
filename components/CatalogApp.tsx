...
export function CatalogApp() {
  ...
  function upsert(input: Omit<Asset, "id" | "createdAt" | "updatedAt"> & { id?: string }) {
    const now = new Date().toISOString();
    if (input.id) {
      persist(
        assets.map((a) =>
          a.id === input.id
            ? { ...a, ...input, id: a.id, createdAt: a.createdAt, updatedAt: now }
            : a,
        ),
      );
  ...
  function remove(id: string) {
    ...
    pushToast("Record removed", () => persist(snapshot));
  }
  ...
  function toggleFavorite(id: string) {
    persist(
      assets.map((a) =>
        a.id === id
          ? { ...a, favorite: !a.favorite, updatedAt: new Date().toISOString() }
          : a,
      ),
    );
  }
    ...
    <div className="page">
      <header className="mast">
        <div className="mast-left">
          ...
          <h1>MyAssets</h1>
          <p className="lede">
            One catalog for components, design references, and web or mobile
            links you will actually reuse.
          </p>
        </div>
        <div className="mast-right">
          ...
          {menuOpen ? (
            <div className="menu" role="menu">
              <button type="button" onClick={() => { exportJson(assets); setMenuOpen(false); }}>
                Export JSON
              </button>
              ...
              <button
                ...
                onClick={() => {
                  persist([]);
                  setMenuOpen(false);
                  pushToast("Catalog cleared", () => persist(assets));
                }}
              >
                Clear catalog
              </button>
      ...
      <footer className="colophon">
        <p>
          Records live in this browser. Export JSON when you want a backup, or
          import to merge catalogs.
        </p>
      </footer>
...
function EditorDialog({
  ...
  categories: string[];
  onClose: () => void;
  onSave: (input: Omit<Asset, "id" | "createdAt" | "updatedAt"> & { id?: string }) => void;
}) {
